...
class TimeTick implements Runnable {
    ...
    /** run thread - exits when alpacamida.TimeTick.stop() is invoked
     */
    public void run() {
        int curticks = 0;
    ...
    /** Stop the alpacamida.TimeTick and have the thread exit
     */
    public void stop() {
        stop = true;
    }
     ...
     *  This effects only updating of the alpacamida.Grid. The alpacamida.Simulation will
     ...
     * @return true of alpacamida.Grid will be updated on ticks, false otherwise
     */
    public boolean paused() {
        return pause;
    }

    /** Set the number of ticks that most go by before calling alpacamida.Grid.update()
     * @param ticks, number of MILLISWAIT (10ms) intervals that must occur
     *               before invoking alpacamida.Grid.update(). non-positive numbers
     *               sets ticks to 1.
